...
public class NetworkController_Lobby : MonoBehaviourPunCallbacks
{
    ...
    [SerializeField]
    private byte maxPlayersInRoom = 6;
    ...
    private void FixedUpdate()
    {
        ...
        if (PhotonNetwork.CurrentRoom != null)
        {
            ...
            {
                PhotonView lobbyPlayerInfo = PhotonView.Find(playerViewIds[player.Value.ActorNumber - 1]);
                ...
                {
                    playerCount.text += "\nPlayer " + player.Key + " (" + player.Value.NickName + "): "
                                        + (lobbyPlayerInfo.gameObject
                                            .GetComponent<Lobby_PlayerInfo>().GetReady()
                                            ? "Listo"
                                            : "No listo");
...
//base.OnPlayerLeftRoom(otherPlayer);
        ...
        bool wasReady = PhotonView.Find(playerViewIds[otherPlayer.ActorNumber - 1]).GetComponent<Lobby_PlayerInfo>()
            .GetReady();
        if (wasReady)
        {
            readyPlayersCount--;
